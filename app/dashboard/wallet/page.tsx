...
export default function WalletPage() {
    ...
    return (
        // ✅ FIX: removed p-4 md:p-6 (layout's <main> already pads); added overflow-x-hidden
        <div className="space-y-4 sm:space-y-6 w-full max-w-full bg-[#0b0e14] text-white overflow-x-hidden">
            ...
            >
                ...
                {/* ✅ FIX: whileTap instead of whileHover; smaller padding on mobile */}
                <motion.div
                    whileTap={{ scale: 0.98 }}
            ...
            {/* Balance Cards */}
            {/* ✅ FIX: gap-3 on mobile; smaller text so +NNNN.NN USDT doesn't wrap */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                {[
                    ...
                    >
                        ...
                        <p className={`text-lg sm:text-2xl font-bold ${item.color} truncate`}>{item.value} <span className="text-xs sm:text-sm font-medium">USDT</span></p>
                    </motion.div>
                ))}
            </div>
            ...
            >
                                        ...
                                        return (
                                            <div key={deposit.id} className="px-3 sm:px-4 py-3 flex items-center justify-between gap-3">
                                                <div className="min-w-0">
                                                    <p className="text-sm text-white truncate">{deposit.amount} USDT</p>
                ...
                {/* ✅ FIX: gap-4 on mobile; inner cards p-4 sm:p-6 */}
                    ...
                    >
                        ...
                        <AnimatePresence>
                            {showQR === 'TRC20' && (
                                ...
                                >
                                    <img src={QR_CODES.TRC20} alt="TRC20 QR" className="w-32 h-32 rounded-lg border border-white/5" />
                                </motion.div>
                            )}
                        </AnimatePresence>

                        {/* ✅ FIX: min-w-0 on the code wrapper so break-all actually works inside flex */}
                        <div className="bg-[#141a24] p-3 rounded-xl border border-white/5 flex items-center justify-between gap-2">
                            <code className="text-xs text-[#8e96a3] break-all font-mono min-w-0">{ADDRESSES.TRC20}</code>
                            <motion.button
                                whileTap={{ scale: 0.9 }}
                                onClick={() => copyAddress(ADDRESSES.TRC20, 'TRC20')}
                                aria-label="Copy address"
                                className="p-2 hover:bg-white/5 rounded text-[#8e96a3] hover:text-white transition shrink-0"
                            >
                        ...
                        <AnimatePresence>
                            {showQR === 'BEP20' && (
                                <motion.div
                                    ...
                                    className="mb-4 flex justify-center"
                                >
                                    <img src={QR_CODES.BEP20} alt="BEP20 QR" className="w-32 h-32 rounded-lg border border-white/5" />
                                </motion.div>
                            )}
                        </AnimatePresence>

                        <div className="bg-[#141a24] p-3 rounded-xl border border-white/5 flex items-center justify-between gap-2">
                            <code className="text-xs text-[#8e96a3] break-all font-mono min-w-0">{ADDRESSES.BEP20}</code>
                            <motion.button
                                whileTap={{ scale: 0.9 }}
            ...
            >
                <div className="bg-[#141a24] p-3 sm:p-4 rounded-xl border border-white/5 min-w-0">
                    ...
                    <p className="text-base sm:text-lg font-bold text-white">{depositHistory.length}</p>
                </div>
                <div className="bg-[#141a24] p-3 sm:p-4 rounded-xl border border-white/5 min-w-0">
