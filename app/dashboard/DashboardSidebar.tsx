...
const menuItems = [
  ...
  { name: 'AI Bot Studio', icon: LineChart, href: '/dashboard/trade' },
  { name: 'Live Markets', icon: Wallet, href: '/dashboard/markets' },
  { name: 'My Wallet', icon: History, href: '/dashboard/deposit' },
  { name: 'Trade History', icon: ShieldCheck, href: '/dashboard/transactions' },
  { name: 'Risk Controls', icon: Settings, href: '/dashboard/risk' },
];
    ...
    <>
      ...
      `}>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {menuItems.map((item) => {
            const IconComponent = item.icon; // <--- THE FIX: Assign it to a variable first
            return (
              <Link key={item.href} href={item.href} onClick={() => setIsMobileMenuOpen(false)}>
                <div className={`flex items-center gap-4 px-4 py-3 rounded-lg transition-all cursor-pointer ${pathname === item.href ? 'bg-[#6366f1]/10 text-[#6366f1] border-l-2 border-[#6366f1]' : 'text-[#8e96a3] hover:text-white hover:bg-white/5'} ${isCollapsed ? 'justify-center px-2' : ''}`}>
                  <IconComponent size={20} /> {/* <--- Use the variable here */}
