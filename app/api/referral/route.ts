...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'get-all-referrals') {
      try {
        console.log('📊 Fetching all referrals with user data...');
        ...
        const { data: userBalances, error: balanceError } = await supabaseAdmin
          .from('user_balances')
        ...
        if (userBalances && !balanceError) {
          console.log(`✅ Found ${userBalances.length} users in user_balances`);
          ...
          console.log('⚠️ No users found in user_balances');
        }
        ...
        if (missingUserIds.length > 0) {
          ...
          const { data: authUsersDirect, error: authErrorDirect } = await supabaseAdmin
            .from('auth.users')
            .select('id, email')
            .in('id', missingUserIds);

          if (authUsersDirect && !authErrorDirect) {
            ...
            console.log(`✅ Found ${authUsersDirect.length} users in auth.users`);
          }
        }
        ...
        console.log(`✅ Mapped ${mappedData.length} referrals with user data`);
    ...
    if (action === 'stats' && user_id) {
      try {
        ...
        const totalEarned = payouts?.reduce((sum, p) => sum + (p.amount_usdt || 0), 0) || 0;
        const pendingPayouts = referrals?.filter(r => r.status === 'pending').length || 0;
        const unreadCount = referrals?.filter(r => r.is_read === false).length || 0;
    ...
    if (action === 'apply-promo' && user_id && promo_code) {
      try {
        ...
        if (promo.usage_limit > 0 && promo.used_count >= promo.usage_limit) {
          ...
          return NextResponse.json({
            success: false,
            error: 'Promo code has reached its usage limit and has been deactivated'
          }, { status: 400 });
        }
        ...
        if (usageError) {
          ...
          return NextResponse.json({
            success: false,
            error: 'Failed to record promo usage: ' + usageError.message
          }, { status: 500 });
        }
        ...
        if (promo.usage_limit > 0 && newUsedCount >= promo.usage_limit) {
        ...
        // ============================================================
        // SEND PROMO CLAIM NOTIFICATION (NEW)
        // ============================================================
        try {
          ...
          if (userData && !userError) {
            ...
            console.log('✅ Promo claim notification sent');
          }
        } catch (promoNotifError) {
          console.error('❌ Promo claim notification error:', promoNotifError);
        }
        ...
        try {
          ...
          console.log('✅ In-app notification created for promo claim');
        } catch (notifError) {
          console.error('Error saving in-app notification:', notifError);
        }
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            message: shouldDeactivate 
              ? `✅ Promo code applied! You received ${bonusAmount} USDT. The promo code has reached its limit and is now deactivated.`
              : `✅ Promo code applied! You received ${bonusAmount} USDT bonus! (${newUsedCount}/${promo.usage_limit || '∞'})`
          }
        });
      } catch (error: any) {
    ...
    if (action === 'mark-read' && user_id) {
      try {
        ...
        console.log(`✅ Marked ${data?.length || 0} referrals as read`);
